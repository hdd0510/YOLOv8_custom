...
from ultralytics.utils import LOCAL_RANK, NUM_THREADS, TQDM, colorstr, DEFAULT_CFG
...
class YOLODataset(BaseDataset):
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            ...
            transforms = Compose([LetterBox(scaleup=True)])
        transforms.append(
            Format(
                bbox_format="xywh",
                normalize=True,
...
class CustomClsDataset(BaseDataset):
    ...
    def verify_images(self):
        ...
        if save_dir := getattr(self.args, "save_dir", None):
            path = Path(f"{save_dir}/{self.mode}_{self.data_name}.cache") # *.cache file path
        else: 
            path = Path(f"{self.mode}_{self.data_name}.cache")
        with contextlib.suppress(FileNotFoundError, AssertionError, AttributeError):
            cache = load_dataset_cache_file(path)  # attempt to load a *.cache file
            assert cache["version"] == DATASET_CACHE_VERSION  # matches current version
            assert cache["hash"] == get_hash([x[0] for x in self.samples])  # identical hash
